...
function background() {
    ...
    var po = [];
    for (var i in td) {
        if (td[i].className != undefined&& td[i].className !== "" && td[i].className !== "control" && td[i].className !== "status" && td[i].className !== "player"&& td[i].className !== "excavations"/*&&td[i].className !== "loca"*/) {
...
function innerContent() {
    for (var i in npc) {
        if (npc[i] != undefined) {
            var npcPosition = "map" + npc[i].position[0] + "_" + npc[i].position[1] + "";
            console.log(npcPosition);
            map[npcPosition].npc = npc[i];
        }
    }
}
...
function contact(){
    for(var i in npc){
        if(npc[i] != undefined){console.log("1")
